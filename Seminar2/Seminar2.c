...
void dezalocareVector(struct Farmacie** vector, int* nrFarmacii) {
	for (int i = 0;i < *nrFarmacii;i++) {
		free((*vector)[i].nume);
	}
	free(*vector);
...
int main() {
	...
	dezalocareVector(&farmacii, &nrFarmacii);
	dezalocareVector(&farmaciiCopiate, &nrFarmaciiCopiate);
	dezalocareVector(&farmaciiMici, &nrFarmaciiMici);
